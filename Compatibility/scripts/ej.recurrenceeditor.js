...
(function ($) {
    var RecurrenceEditor = (function (_super) {
        ...
        RecurrenceEditor.prototype._renderRecurrenceContent = function () {
            ...
            if (this._mediaQuery) {
                ...
                for (var weekcount = 0; weekcount < this._dayShortNames.length; weekcount++) {
                    $recurCont += "<td><div class='e-weekday " + this._dayShortNames[weekcount] + "'><input id='" + this._id + "_" + this._dayNames[weekcount] + "' class='weekdays e-weekly" + this._dayNames[weekcount] + "'name='weekdays_ " + this._dayNames[weekcount] + "' type='checkbox'/><label class='e-textmargin' for='" + this._id + "_" + this._dayNames[weekcount] + "'>" + this._dayShortNames[weekcount] + "</label></div></td>";
                }
        ...
        RecurrenceEditor.prototype._dayOfWeekInMonth = function (strDate, dayName, weekIndex) {
            ...
            var monthDate;
            if (weekIndex == -1) {
                var curDate = new Date(tempDate.getFullYear(), tempDate.getMonth() + 1, 0);
                while (curDate.getDay() != dayIndex) {
                    curDate = new Date(curDate.setDate(curDate.getDate() - 1));
                }
                monthDate = new Date(curDate.toString());
            }
            else {
                do {
                    var count = 0;
                    monthDate = new Date(tempDate.getFullYear(), tempDate.getMonth(), 1);
                    while (true) {
                        if (monthDate.getDay() === dayIndex) {
                            if (weekIndex != 5) {
                                ...
                                if (tempDate.getMonth() != monthDate.getMonth()) {
                ...
                } while (monthDate.getTime() < strDate.getTime());
            }
            return monthDate;
        };
        RecurrenceEditor.prototype._dayOfMonthInYear = function (strDate, dayName, weekIndex) {
            var dayIndex = this._dayNamesValue.indexOf(dayName);
            var monthIndex = this._rRule.month - 1;
            strDate.setHours(0, 0, 0, 0);
            var tempDate = new Date(strDate);
            var yearDate;
            if (weekIndex == -1) {
                ...
                yearDate = new Date(curDate.toString());
            }
            else {
                do {
                    var count = 0;
                    yearDate = new Date(tempDate.getFullYear(), monthIndex, 1);
                    while (true) {
                        if (yearDate.getDay() === dayIndex) {
                            if (weekIndex != 5) {
                                if (++count == weekIndex) {
                                    break;
                                }
                                ...
                                if (yearDate.getMonth() != monthIndex) {
                ...
                } while (yearDate.getTime() < strDate.getTime());
            }
            return yearDate;
        };
            ...
            while (day_start <= recurEndDate) {
                switch (this._rRuleFreq) {
                    ...
                    case "MONTHLY":
                        ...
                        var recurSplit = null;
                        this.recurrenceRuleSplit(this._recRule, recurSplit);
                        if (ej.isNullOrUndefined(this._rRule.setPositions)) {
                            var date = new Date(day_start.toString());
                            date.setDate(this._rRule.monthDay);
                            if (date <= recurEndDate && date >= day_start) {
                                wTrue = true;
                            }
                        }
                        else {
                            var day = this._dayOfWeekInMonth(day_start, this._rRule.weekDays, this._rRule.setPositions);
                            if (this._dayOfWeekInMonth(day_start, this._rRule.weekDays, this._rRule.setPositions) <= recurEndDate && day.getMonth() == day_start.getMonth()) {
                                wTrue = true;
                            }
                        }
        ...
        RecurrenceEditor.prototype.recurrenceDateGenerator = function (recurrenceString, strDate, recurExDate) {
            ...
            if (ej.isNullOrUndefined(this._rRule.recurEditId)) {
                ...
                switch (this._rRule.freq) {
                    case "WEEKLY":
                        if (!ej.isNullOrUndefined(this._rRule.weekDays)) {
                            ...
                            var col = ["SU", "MO", "TU", "WE", "TH", "FR", "SA"];
                            byDay = col[day_start.getDay()];
                        }
                        break;
                    case "MONTHLY":
                        if (!ej.isNullOrUndefined(this._rRule.monthDay)) {
                            ...
                            var dayInSameMonth = this._rRule.monthDay >= new Date(day_start).getDate();
                            var startMonth = new Date(day_start).getMonth();
                            var isLastDate = false;
                            while (new Date(day_start).getDate() != this._rRule.monthDay && !isLastDate) {
                                var sameMonth = dayInSameMonth ? new Date(day_start).getMonth() == startMonth : true;
                                if (sameMonth) {
                                    day_start = new Date(day_start.setDate(day_start.getDate() + 1));
                                }
                                else {
                ...
                while (day_start <= recurEndDate) {
                    (this._rRule.freq == "MONTHLY" && !ej.isNullOrUndefined(this._rRule.setPositions)) && (day_start = this._dayOfWeekInMonth(day_start, this._rRule.weekDays, this._rRule.setPositions));
                    (this._rRule.freq == "YEARLY" && !ej.isNullOrUndefined(this._rRule.setPositions)) && (day_start = this._dayOfMonthInYear(day_start, this._rRule.weekDays, this._rRule.setPositions));
                    ...
                    switch (this._rRule.freq) {
                        case "DAILY":
                            if ((byDay.length > 0 && byDay.indexOf(this._dayNamesValue[value.getDay()]) > -1) || byDay.length === 0) {
                        ...
                        case "WEEKLY":
                            ...
                            if (byDay.length - 1 != byDay.indexOf(curDay)) {
                                ...
                                day_start = new Date(day_start.setDate(day_start.getDate() + (_interval * 7) - this._firstDayOfWeek - 1));
                                day_start = this._weeklyDayFind(day_start, byDay);
                            }
                            break;
                        case "MONTHLY":
                        ...
                        case "YEARLY":
                            RecDateCollection.push(new Date(value.toString()).getTime());
                            var start_day = ej.isNullOrUndefined(this._rRule.setPositions) ? day_start.getDate() : 1;
                            day_start = new Date((day_start.getFullYear() + _interval), (this._rRule.month - 1), start_day);
                            break;
                    }
                    _count++;
