...
(function ($, ej, undefined) { 
    ...
    ej.widget("ejDropDownList", "ej.DropDownList", {
        ...
        selectItemByText: function (val, ignoreCasing) {
            var valTexts= (val.split(",").length != 0) ? val.split(","):val;
            var checkTexts = []; checkStringText = [];
            if(valTexts.length>0){
                ...
                if (this.model.itemsCount < checkTexts[checkTexts.length - 1]) {
                    if (this.model.selectedIndices.length == 0 && checkTexts.length == 1){
                        this.model.selectedIndex = checkTexts[0];
        ...
        _getQuery: function (isLocal) {
            var remoteUrl, mapper = this.model.fields, queryManager = ej.Query(), midRange,addValue;
			...
			{
                midRange = Math.round(this.model.itemsCount / 2); addValue = (this.model.itemsCount % 2 ==0)? 0 : 1;
                 this._dataFIndex = this._dataLIndex = this.model.selectedIndex-midRange; 
				queryManager.skip(this.model.selectedIndex+midRange - this.model.itemsCount-addValue).take(this.model.itemsCount);
        ...
        _renderRemaining: function () {
           ...
           if(!ej.isNullOrUndefined(this.popupListWrapper)){ 
            ...
            if(!this.popupList.hasClass("e-scroller")){
        ...
        _onScroll: function (e) {
            ...
            if (proxy._isPopOpenedFirst && !ej.isNullOrUndefined(proxy.activeItem)){
                window.setTimeout(function () {
                    proxy.activeItem[0].scrollIntoView();
                    window.setTimeout(function () {
            proxy.activeItem[0].scrollIntoView();
