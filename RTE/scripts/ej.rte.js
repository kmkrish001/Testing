...
(function ($, ej, undefined) { 
    ...
    ej.widget("ejRTE", "ej.RTE", {
        ...
        _contextMenuClick: function (e) {
            ...
            switch (action) {
                ...
                case "imageproperties":
                    ...
                    this._isimagePropertyChecked = true;
                    this._imageManager();
                    this._isimagePropertyChecked = false;
                    break;
                case "addcolumnleft":
        ...
        _disableResizeObj: function (value) {
            if (!value) {
                ...
                this._off($(this._getDocument()), "mouseover", 'table', this._tableMouseOver)
            ...
            if (this.model.imageBrowser.filePath != "") {
                ...
                this._explorerObj = this._imgDialog.find("div.e-rte-imgBrowser").ejFileExplorer({
                    ...
                    open: function(args){
                        proxy._loaoded = 0;
                        proxy._images = [];
                        if (args.itemType == "Directory") {
                            proxy._imageList = [];
                            proxy._currentSelectedImage = null;
                            proxy._images = $(this.element.find("#" + this.element[0].id + "_tileWrapper")).find("img");
                            if (proxy._images.length > 0) {
                    ...
                    getImage: function(args){
                        proxy._loaoded = proxy._loaoded + 1;
                        var itemDetails = {path: args.path, height: args.element.height, width: args.element.width};
                        proxy._imageList.push(itemDetails);
                        if(proxy._loaoded == proxy._images.length){
                            this._waitingPopup.hide();
                            proxy._loaoded = 0;
                        }
                    }
                }).data('ejFileExplorer');
                if(this._isimagePropertyChecked){
                    this._restrictSelected = true;
                    this._explorerObj.option("selectedFolder", $(this._imgURL[0]).val().substr(0,$(this._imgURL[0]).val().lastIndexOf("/")));
                    this._restrictSelected = false;
                }
        ...
        _disableEnableInsertButton: function (isDisabled) {
            if (isDisabled) {
                document.getElementById(this._rteId + "_img_insert").disabled = true;
                document.getElementById(this._rteId + "_img_insert").classList.add("e-disable");
                if (!this._isimagePropertyChecked) {
                    this._imgWidth = "";
                    this._imgHeight = "";
                ...
                document.getElementById(this._rteId + "_img_insert").disabled = false;
                document.getElementById(this._rteId + "_img_insert").classList.remove("e-disable");
            }
        },
        _UpdateImgDetails: function (e) {
            if (e.nodeType == "Directory") {
                this._disableEnableInsertButton(true);
            } else {
                this._disableEnableInsertButton(false);
            }
            if(this._restrictSelected) return;
            if ($(this._imgURL).hasClass('e-error')) this._imgURL.removeClass("e-error");
            if (e.path === "") {
                this._imgURL.val("http://");
                this._disableEnableInsertButton(true);
            }
            else this._imgDialog.find("div.e-rte-imageTab").find('.e-rte-imgUrl').val(e.path + (e.name[0] ? e.name[0] : ""));
            if(e.model.fileTypes.indexOf("*" + e.name[0].substr(e.name[0].lastIndexOf("."))) >= 0){
                this._currentSelectedImage = this._explorerObj._tileViewWrapper.find(".e-active").find("img").attr("src");
                for (var a = 0; a < this._imageList.length ; a++) {
                    if(this._imageList[a].path == this._currentSelectedImage){
                        this._imgWidth = this._imageList[a].width;
                        this._imgHeight = this._imageList[a].height;
                    }
                }
                if (this.model.showDimensions) {
                    this._imgDialog.find(".e-rte-imgX").val(this._imgWidth);
        ...
        _clearImgFields: function () {
            ...
            proxy = this;
            this._imgURL.on("blur keydown", function (e) {
                if (proxy._imgURL.val() == "") {
                    proxy._imgURL.addClass("e-error");
                    proxy._disableEnableInsertButton(true);
                } else {
                    proxy._imgURL.removeClass("e-error");
                    if(proxy._imgURL.val() != "http://"){
                        proxy._disableEnableInsertButton(false);
                    }
                }
            });
        ...
        _eTblManager: function () {
            ...
            if (!ej.isNullOrUndefined(selectNode) && selectNode.tagName.toLowerCase() === "td") {
                var parentTable = $(selectNode).closest('tbody').closest('table');
                 this._getTableObj("Height").val(parentTable[0].offsetHeight + ((parentTable[0].style.borderWidth !=="") ? parseFloat(parentTable[0].style.borderWidth) : 0));
                 this._getTableObj("Width").val(parentTable[0].offsetWidth + ((parentTable[0].style.borderWidth !=="") ? parseFloat(parentTable[0].style.borderWidth) : 0));
                ...
                this._getTableObj("RHeight").val(parentRow[0].offsetHeight);
                ...
                this._getTableObj("ClWidth").val(selectNode.offsetWidth);
                ...
                this._tableStyles = {table: {width: Number(this._getTableObj("Width").val()), height: Number(this._getTableObj("Height").val())}, rows:{height: Number(this._getTableObj("RHeight").val())}, column: {width: Number(this._getTableObj("ClWidth").val())} };
            }
        },
        ...
        _imageManager: function () {
            ...
            if (selectNode && !ej.isNullOrUndefined(selectNode)) {
                ...
                if (selectNode.tagName.toLowerCase() === "img") {
                    ...
                    var selectedImgstyle = $(selectNode).attr('style').replace('outline: rgb(92, 92, 92) dashed 1px;','');
                    this._imageStyle.val(selectedImgstyle);
                }
            }
        ...
        _renderimport: function () {
            ...
            divEle.ejUploadbox({
                ...
                success: function (args) {
                    ...
                    iframe[0].insertAdjacentHTML('afterbegin', htmlText);
                }
            });
       ...
       _onBGColor: function (color) {
        ...
        _oneditTable: function () {
            ...
            if (!this._customTableValidation) {
                ...
                (this._tableStyles.table.width !== Number(tableWid)) ? parentTable.css({'width':isNaN(Number(tableWid))?tableWid:tableWid+"px"}): null;
                (this._tableStyles.table.height !== Number(tableHgt)) ? parentTable.css({'height': isNaN(Number(tableHgt))?tableHgt:tableHgt+"px"}): null;
                ...
                (this._tableStyles.rows.height !== Number(rowHgt)) ? parentRow.css({'height': isNaN(Number(rowHgt))?rowHgt:rowHgt+"px"}): parentRow.css({'height': 'auto'});
                $(selectedNode).attr({
                ...
                (this._tableStyles.column.width !== Number(cellWid)) ? parentRow.css({'width': isNaN(Number(cellWid))?cellWid:cellWid+"px"}): parentRow.css({'width': 'auto'});
                this._eTblDialog.ejDialog("close");
                this._tableResizeUp();
            }
            ...
            try {
                ...
                if (this.model.showToolbar) {
                    ...
                    for (var i = 0; i < this._alignItems.length; i++) {
                        var checkAlign = (this._isIE() || ej.browserInfo().name === "mozilla")?this._alignStatus($.trim(this._alignItems[i].toLowerCase())) : this._getCommandStatus($.trim(this._alignItems[i].toLowerCase()));
                        if (checkAlign) {
        ...
        _ieParents: function (range) {
            var parentElement = [];
            var rangeval = this._isIE8() ? range.duplicate() : range.cloneRange();
        ...
        _setBackupData: function () {
            ...
            var save = this._save(range, this.getDocument());
            var changeEle = {text: htmlTxt, range: save }
            }
            ...
            if (htmlTxt != this._backupArray[bkuplen - 1] && this.model.undoStackLimit > 0)
                (!this._isIE()) ? this._backupArray.push(changeEle): this._backupArray.push(htmlTxt) ;
        ...
        _validateMaxLength: function (e) {
            if (this.model.maxLength != null) {
                ...
                if (len <= usertext.length) {
                    if (!((e.keyCode < 47 && e.keyCode != 32 ) || e.ctrlKey && e.keyCode == 65 || e.ctrlKey && e.keyCode == 67 || e.ctrlKey && e.keyCode == 86 || e.ctrlKey && e.keyCode == 88)) {
        ...
        _save: function (range, docElement) {
            range = (range) ? range.cloneRange() : this.getRange(docElement);
            var body = docElement.body;
            var range = range.cloneRange();
            this._undoSave = {range: range, rootNode: this._rootNodes(range), body: body, 
                startContainer: this._getNodeArray(range, range.startContainer, true),
                endContainer: this._getNodeArray(range, range.endContainer, false),
                startOffset: range.startOffset, endOffset: range.endOffset, html: body.innerHTML};
            return this._undoSave;
        },
        _rootNodes: function(range){
            return (9 === range.startContainer.nodeType) ? range.startContainer : range.startContainer.ownerDocument;
        },
        _getNodeArray: function (range, node, isStart, root) {
        var array = [];
        ((isStart) ? (this.startNodeName = []) : (this.endNodeName = []));
        for (; node !== (root ? root : this._rootNodes(range)); null) {
            (isStart) ? this.startNodeName.push(node.nodeName.toLowerCase()) : this.endNodeName.push(node.nodeName.toLowerCase());
            array.push(this._getIndex(node));
            node = node.parentNode;
        }
        return array;
        },
    ...
    _restore: function () {
        var range = this._undoSave.range.cloneRange();
        range = this._setRangePoint(range, true, this._undoSave.startContainer, this._undoSave.startOffset);
        range = this._setRangePoint(range, false, this._undoSave.endContainer, this._undoSave.endOffset);
        var docElement = this._rootNodes(range);
        var selection = docElement.defaultView.getSelection();
        selection.removeAllRanges();
        selection.addRange(range);
        this._save(range, docElement);
        return range;
    },
    _setRangePoint: function (range, isvalid, num, size) {
        var node = this._undoSave.rootNode;
        ...
        _getRedoContent: function () {
            if (this.model.enableHtmlEncode)
                return (!this._isIE()) ? this._decode(this._backupArray[this._undoRedoPosition + 1].text) : this._decode(this._backupArray[this._undoRedoPosition + 1]);
            else
                return (!this._isIE()) ? this._backupArray[this._undoRedoPosition + 1].text : this._backupArray[this._undoRedoPosition + 1];
        },
        ...
        _tableMouseOver: function (e) {
            if (!this.model.enableRTL && e.target.nodeName == "TABLE" || (ej.browserInfo().name === "mozilla" && e.target.nodeName == "TD")) {
        ...
        _iframeKeyUp: function (e) {
            ...
            var keyValue = e.key;
            if (((keyValue !== 'shift' && !e.ctrlKey) && keyValue.length === 1 || allowedKeys) || (this.editorMode === 'Markdown'
            && ((keyValue !== 'shift' && !e.ctrlKey) && keyValue.length === 1 || allowedKeys)) && !this.inlineMode.enable) {
                this._debounce(300);
            }
        ...
        _imgBoxMouseMove: function (e) {
            if (this._commonBoxDown) {
                ...
                boxEle[0].style.width = _width + "%";
                boxEle[0].style.height = _height + "%";
        ...
        _iframePaste: function (e) {
			...
			if(e){
				...
				try {
					var val = ej.isNullOrUndefined(window.clipboardData) ? clipboardData.getData("text/plain") : window.clipboardData.getData("Text");
					if (val.length != 0) {
						...
						var fileName;
						if (blob != null) {
							fileName = blob.type.lastIndexOf("/");
							if (fileName != -1) {
								fileName = blob.type.substr(0, blob.type.lastIndexOf('/'));
							}
						}
						if (ej.browserInfo().name != "mozilla" && fileName != "image") {
        ...
        _wireEvents: function () {
            ...
            if (this._getDocument() != "") {
                ...
                ._on(_iframe, "mouseover", 'table', this._tableMouseOver);
        ...
        _unwireEvents: function () {
            ...
            ._off(_iframe, "mouseover", 'table', this._tableMouseOver);
        ...
        _onChange: function () {
           var _prevhtml = this._decode(this._encode($.trim(this.value())));
            if (this.model.enableXHTML) {
                this._updateXhtml();
                if (this.model.enableHtmlEncode)
        ...
        refresh: function () {
            ...
            this._showHideContextMenu();
			if(ej.clipboardCleaner && !this._isIE8() && this.model.pasteCleanupSettings && (this.model.pasteCleanupSettings.cleanElements || this.model.pasteCleanupSettings.removeStyles || this.model.pasteCleanupSettings.cleanCSS || this.model.pasteCleanupSettings.listConversion))
			{
			   this._pasteCleaner = new ej.clipboardCleaner(this._getDocument(),this._iframePaste,this);
			   this._pasteCleaner.filterOptions = this.model.pasteCleanupSettings;
			}
        },
