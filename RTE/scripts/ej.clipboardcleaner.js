var clipboardCleaner = (function () {
    ...
    clipboardCleaner.prototype._insertContent = function (args, clipboardArgs) {
        ...
        this.callback.call(this.env, clipboardArgs);
    ...
    clipboardCleaner.prototype._processContent = function (args, state) {
        ...
        if (patern.test(this.htmlContent) && state) {
            ...
            if (this.insertCleanedContent)
                this._insertContent(null, args);
        }
        else {
            this.syncLoad = true;
            elm.innerHTML = this.htmlContent;
            if (this.filterOptions.cleanCSS) {
                elm.innerHTML = this.htmlContent.replace(/<!--StartFragment-->([^<]*(?:<(?!!--(?:Start|End)Fragment-->)[^<]*)*)<!--EndFragment-->/g, '$1');
                this._cleanStyles(elm);
                this._flush(false);
            }
            ...
            this.Container = elm;
            if (this.insertCleanedContent)
                this._insertContent(!nonBlock, args);
        }
    };
